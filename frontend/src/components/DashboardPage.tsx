    ...
    <section className="grid gap-10 bg-panel px-shell py-feature-y text-ink max-phone:px-mobile-shell max-phone:py-panel-mobile" aria-labelledby="admin-title">
      <header className="grid gap-4 border-b border-field-line pb-8 md:grid-cols-[1fr_auto] md:items-end">
        ...
        <div className="grid grid-cols-2 gap-px border border-field-line bg-field-line text-center">
          <div className="bg-cream px-5 py-4"><strong className="block font-display text-3xl text-ink">{activeTables}</strong><span className="font-mono text-[10px] uppercase tracking-label text-panel-copy">Mesas activas</span></div>
          <div className="bg-cream px-5 py-4"><strong className="block font-display text-3xl text-ink">{confirmedReservations}</strong><span className="font-mono text-[10px] uppercase tracking-label text-panel-copy">Confirmadas</span></div>
        </div>
          ...
          <section className="grid gap-6 border-t border-field-line pt-10" aria-labelledby="reservations-title">
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
              <div className="min-w-0">
                <div className="hidden border border-field-line bg-cream xl:block">
                  <table className="w-full table-fixed text-left text-sm"><caption className="sr-only">Reservas del restaurante</caption><thead className="border-b border-field-line font-mono text-[10px] uppercase tracking-label text-label"><tr><th className="w-[18%] px-3 py-3">Fecha</th><th className="w-[23%] px-3 py-3">Cliente</th><th className="w-[10%] px-3 py-3">Mesa</th><th className="w-[9%] px-3 py-3">Personas</th><th className="w-[20%] px-3 py-3">Estado</th><th className="w-[20%] px-3 py-3">Acciones</th></tr></thead><tbody>{reservations.map((reservation) => <tr className="border-b border-field-line align-top last:border-0" key={reservation.id}><td className="px-3 py-4 text-ink"><strong className="block">{formatDate(reservation.date)}</strong><span className="text-xs text-panel-copy">{formatTime(reservation.startTime)} — {formatTime(reservation.endTime)}</span></td><td className="px-3 py-4 text-ink"><strong className="block truncate">{reservation.user.name}</strong><span className="block truncate text-xs text-panel-copy">{reservation.user.email}</span></td><td className="px-3 py-4 text-panel-copy">Mesa {reservation.table.number}</td><td className="px-3 py-4 text-panel-copy">{reservation.guests}</td><td className="px-3 py-4"><span className={`inline-block px-2 py-1 font-mono text-[9px] uppercase tracking-label ${statusClass(reservation.status)}`}>{reservation.status}</span><span className="mt-2 block truncate font-mono text-[9px] text-panel-copy">{reservation.confirmationCode}</span></td><td className="px-3 py-4"><div className="grid gap-2"><button aria-label={`Editar reserva ${reservation.confirmationCode}`} className="w-full cursor-pointer border border-brown-line px-2 py-2 font-mono text-[10px] uppercase tracking-link text-label transition-colors hover:border-ink hover:bg-ink hover:text-cream disabled:cursor-not-allowed disabled:opacity-40" disabled={reservation.status !== 'CONFIRMED'} onClick={() => editReservation(reservation)} type="button">Editar</button><button aria-label={`Cancelar reserva ${reservation.confirmationCode}`} className="w-full cursor-pointer border border-field-line px-2 py-2 font-mono text-[10px] uppercase tracking-link text-panel-copy transition-colors hover:border-label hover:text-label disabled:cursor-not-allowed disabled:opacity-40" disabled={reservation.status !== 'CONFIRMED'} onClick={() => void cancelAdminReservation(reservation)} type="button">Cancelar</button></div></td></tr>)}</tbody></table>
                </div>
                <div className="grid gap-3 xl:hidden">
                  {reservations.map((reservation) => <article className="grid gap-4 border border-field-line bg-cream p-4 text-ink" key={reservation.id}>
                    <div className="flex items-start justify-between gap-4"><div><strong className="block">{formatDate(reservation.date)}</strong><span className="text-xs text-panel-copy">{formatTime(reservation.startTime)} — {formatTime(reservation.endTime)}</span></div><span className={`shrink-0 px-2 py-1 font-mono text-[9px] uppercase tracking-label ${statusClass(reservation.status)}`}>{reservation.status}</span></div>
                    <div><strong className="block">{reservation.user.name}</strong><span className="block truncate text-xs text-panel-copy">{reservation.user.email}</span></div>
                    <div className="grid grid-cols-2 gap-3 border-t border-field-line pt-3 text-sm"><span className="text-panel-copy">Mesa <strong className="text-ink">{reservation.table.number}</strong></span><span className="text-panel-copy">Personas <strong className="text-ink">{reservation.guests}</strong></span></div>
                    <p className="m-0 truncate font-mono text-[10px] text-panel-copy">{reservation.confirmationCode}</p>
                    <div className="grid gap-2 sm:grid-cols-2"><button aria-label={`Editar reserva ${reservation.confirmationCode}`} className="w-full cursor-pointer border border-brown-line px-3 py-2 font-mono text-[10px] uppercase tracking-link text-label transition-colors hover:border-ink hover:bg-ink hover:text-cream disabled:cursor-not-allowed disabled:opacity-40" disabled={reservation.status !== 'CONFIRMED'} onClick={() => editReservation(reservation)} type="button">Editar reserva</button><button aria-label={`Cancelar reserva ${reservation.confirmationCode}`} className="w-full cursor-pointer border border-field-line px-3 py-2 font-mono text-[10px] uppercase tracking-link text-panel-copy transition-colors hover:border-label hover:text-label disabled:cursor-not-allowed disabled:opacity-40" disabled={reservation.status !== 'CONFIRMED'} onClick={() => void cancelAdminReservation(reservation)} type="button">Cancelar reserva</button></div>
                  </article>)}
                </div>
                {reservations.length === 0 && <p className="border border-field-line bg-cream px-5 py-10 text-center text-sm text-panel-copy">No hay reservas para mostrar.</p>}
              </div>
