...
export function DashboardPage({ user }: DashboardPageProps) {
  ...
  const [activeSection, setActiveSection] = useState<'tables' | 'reservations'>('reservations');
  const isAdmin = user.role === 'ADMIN';

  const tableFormRef = useRef<HTMLFormElement>(null);
  ...
  function editTable(table: AdminTable) {
    ...
    tableFormRef.current?.scrollIntoView({ behavior: 'smooth' });
  }
  ...
  async function removeTable(table: AdminTable) {
    if (!isAdmin || !window.confirm(`¿Desactivar la mesa ${table.number}?`)) return;
    try {
  ...
  async function cancelAdminReservation(reservation: AdminReservation) {
    if (!isAdmin || !window.confirm(`¿Cancelar la reserva ${reservation.confirmationCode}?`)) return;
    try {
  ...
  return (
    <section className="grid gap-10 bg-panel px-shell py-feature-y text-ink max-phone:px-mobile-shell max-phone:py-panel-mobile" aria-labelledby="admin-title">
      <header className="grid gap-4 border-b border-field-line pb-8 md:grid-cols-[1fr_auto] md:items-end">
        <div>
          <p className="font-mono text-xs uppercase tracking-brand text-label">Panel de gestión</p>
      ...
      <nav className="flex gap-1 border border-line bg-ink p-1 mb-6" aria-label="Navegación del panel">
        <button
          className={`flex-1 font-mono text-xs uppercase tracking-brand py-3 transition-colors ${activeSection === 'reservations' ? 'bg-gold text-ink font-semibold' : 'text-cream hover:text-gold'}`}
          onClick={() => setActiveSection('reservations')}
        >
          Reservas
        </button>
        <button
          className={`flex-1 font-mono text-xs uppercase tracking-brand py-3 transition-colors ${activeSection === 'tables' ? 'bg-gold text-ink font-semibold' : 'text-cream hover:text-gold'}`}
      ...
      {loading ? <p className="py-10 text-panel-copy" role="status">Cargando la operación del restaurante…</p> : (
        <>
            ...
            <section className="grid gap-6" aria-labelledby="tables-title">
              ...
              <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]">
                ...
                {isAdmin && <form className="grid content-start gap-4 border border-brown-line bg-panel p-5 text-ink" onSubmit={submitTable} ref={tableFormRef}>
                  ...
                  <div className="flex gap-3"><Button type="submit">{editingTableId ? 'Guardar cambios' : 'Crear mesa'} <span className="ml-2 text-gold">→</span></Button>{editingTableId && <button className="cursor-pointer font-mono text-[10px] uppercase tracking-link text-label underline underline-offset-4 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-label" onClick={resetTableForm} type="button">Cancelar</button>}</div>
                </form>}
              </div>
            </section>
          )}
          ...
          {activeSection === 'reservations' && (
            <section className="grid gap-6 pt-10" aria-labelledby="reservations-title">
              ...
              <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
                <div className="min-w-0">
                  <div className="hidden border border-line bg-ink xl:block">
                    <table className="w-full table-fixed text-left text-sm"><caption className="sr-only">Reservas del restaurante</caption><thead className="border-b border-line font-mono text-[10px] uppercase tracking-label text-gold"><tr><th className="w-[18%] px-3 py-3">Fecha</th><th className="w-[23%] px-3 py-3">Cliente</th><th className="w-[10%] px-3 py-3">Mesa</th><th className="w-[9%] px-3 py-3">Personas</th><th className="w-[20%] px-3 py-3">Estado</th>{isAdmin && <th className="w-[20%] px-3 py-3">Acciones</th>}</tr></thead><tbody>{reservations.map((reservation) => <tr className="border-b border-line align-top last:border-0" key={reservation.id}><td className="px-3 py-4 text-cream"><strong className="block">{formatDate(reservation.date)}</strong><span className="text-xs text-muted">{formatTime(reservation.startTime)} — {formatTime(reservation.endTime)}</span></td><td className="px-3 py-4"><strong className="block text-cream">{reservation.user.name}</strong><span className="truncate text-xs text-muted">{reservation.user.email}</span></td><td className="px-3 py-4 text-cream">{reservation.table.number}</td><td className="px-3 py-4 text-cream">{reservation.guests}</td><td className="px-3 py-4"><span className={`inline-block px-2 py-1 font-mono text-[9px] uppercase tracking-label ${statusClass(reservation.status)}`}>{reservation.status}</span></td>{isAdmin && <td className="px-3 py-4"><div className="grid gap-2"><button aria-label={`Editar reserva ${reservation.confirmationCode}`} className="cursor-pointer border border-brown-line px-2 py-1 font-mono text-[10px] uppercase tracking-link text-gold transition-colors hover:border-gold hover:bg-gold hover:text-ink disabled:cursor-not-allowed disabled:opacity-40" disabled={reservation.status !== 'CONFIRMED'} onClick={() => editReservation(reservation)} type="button">Editar</button><button aria-label={`Cancelar reserva ${reservation.confirmationCode}`} className="cursor-pointer border border-line px-2 py-1 font-mono text-[10px] uppercase tracking-link text-muted transition-colors hover:border-gold hover:text-gold disabled:cursor-not-allowed disabled:opacity-40" disabled={reservation.status !== 'CONFIRMED'} onClick={() => cancelAdminReservation(reservation)} type="button">Cancelar</button></div></td>}</tr>)}</tbody></table>
                  </div>
                  <div className="grid gap-3 xl:hidden">
                    {reservations.map((reservation) => <article className="grid gap-4 border border-line bg-ink p-4 text-cream" key={reservation.id}>
                      ...
                      <div className="grid grid-cols-2 gap-3 border-t border-line pt-3 text-sm"><span className="text-muted">Mesa <strong className="text-cream">{reservation.table.number}</strong></span><span className="text-sm text-muted">Personas <strong className="text-cream">{reservation.guests}</strong></span></div>
                      <p className="m-0 truncate font-mono text-[10px] text-muted">{reservation.confirmationCode}</p>
                      {isAdmin && <div className="grid gap-2 sm:grid-cols-2"><button aria-label={`Editar reserva ${reservation.confirmationCode}`} className="w-full cursor-pointer border border-brown-line px-3 py-2 font-mono text-[10px] uppercase tracking-link text-gold transition-colors hover:border-gold hover:bg-gold hover:text-ink disabled:cursor-not-allowed disabled:opacity-40" disabled={reservation.status !== 'CONFIRMED'} onClick={() => editReservation(reservation)} type="button">Editar reserva</button><button aria-label={`Cancelar reserva ${reservation.confirmationCode}`} className="w-full cursor-pointer border border-line px-3 py-2 font-mono text-[10px] uppercase tracking-link text-muted transition-colors hover:border-gold hover:text-gold disabled:cursor-not-allowed disabled:opacity-40" disabled={reservation.status !== 'CONFIRMED'} onClick={() => cancelAdminReservation(reservation)} type="button">Cancelar reserva</button></div>}
                    </article>)}
                  </div>
                ...
                {isAdmin && <form className="grid content-start gap-4 border border-brown-line bg-panel p-5 text-ink" onSubmit={submitReservation} ref={reservationFormRef}>
