...
export function DashboardPage({ user }: DashboardPageProps) {
  ...
  return (
    <section className="grid gap-10 bg-ink px-shell py-feature-y text-cream max-phone:px-mobile-shell max-phone:py-panel-mobile" aria-labelledby="admin-title">
      <header className="grid gap-4 border-b border-line pb-8 md:grid-cols-[1fr_auto] md:items-end">
        <div>
          <p className="font-mono text-xs uppercase tracking-brand text-gold">Panel administrador</p>
          <h1 id="admin-title" className="mt-3 font-display text-booking font-medium leading-heading text-cream">Control del salón</h1>
          <p className="mt-4 max-w-booking text-muted">Hola, {user.name}. Gestiona mesas y reservas con las reglas del restaurante.</p>
        </div>
        <div className="grid grid-cols-2 gap-px border border-line bg-line text-center">
          <div className="bg-note px-5 py-4"><strong className="block font-display text-3xl text-gold">{activeTables}</strong><span className="font-mono text-[10px] uppercase tracking-label text-muted">Mesas activas</span></div>
          <div className="bg-note px-5 py-4"><strong className="block font-display text-3xl text-gold">{confirmedReservations}</strong><span className="font-mono text-[10px] uppercase tracking-label text-muted">Confirmadas</span></div>
        </div>
        <button className="cursor-pointer justify-self-end font-mono text-[10px] uppercase tracking-link text-gold underline underline-offset-4 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gold md:col-start-2" onClick={() => void handleLogout()} type="button">Cerrar sesión</button>
      </header>

      {status && <p className={status.tone === 'error' ? 'border border-red-300 bg-note px-4 py-3 text-sm text-red-200' : 'border border-gold bg-note px-4 py-3 text-sm text-gold'} role={status.tone === 'error' ? 'alert' : 'status'}>{status.text}</p>}
      {loading ? <p className="py-10 text-muted" role="status">Cargando la operación del restaurante…</p> : (
        <>
          <section className="grid gap-6" aria-labelledby="tables-title">
            <div className="flex items-end justify-between gap-4 max-phone:flex-col max-phone:items-start">
              <div><p className="font-mono text-xs uppercase tracking-brand text-gold">Operación</p><h2 id="tables-title" className="mt-2 font-display text-4xl font-medium tracking-heading text-cream">Mesas</h2></div>
              <p className="text-sm text-muted">{tables.length} configuradas · {activeTables} activas</p>
            </div>
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]">
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4">
                {tables.map((table) => <article className={`min-h-40 border p-4 ${table.isActive ? 'border-brown-line bg-note text-cream' : 'border-line bg-note text-muted opacity-80'}`} key={table.id}>
                  <div className="flex items-start justify-between gap-2"><span className="font-display text-3xl">{table.number}</span><span className={`font-mono text-[9px] uppercase tracking-label ${table.isActive ? 'text-gold' : 'text-muted'}`}>{table.isActive ? 'Activa' : 'Inactiva'}</span></div>
                  <p className="mt-3 text-sm text-muted">{table.capacity} {table.capacity === 1 ? 'persona' : 'personas'}</p>
                  <div className="mt-4 grid gap-2"><button aria-label={`Editar mesa ${table.number}`} className="w-full cursor-pointer border border-brown-line px-2 py-2 font-mono text-[10px] uppercase tracking-link text-gold transition-colors hover:border-gold hover:bg-gold hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gold" onClick={() => editTable(table)} type="button">Editar</button>{table.isActive && <button aria-label={`Desactivar mesa ${table.number}`} className="w-full cursor-pointer border border-line px-2 py-2 font-mono text-[10px] uppercase tracking-link text-muted transition-colors hover:border-gold hover:text-gold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gold" onClick={() => void removeTable(table)} type="button">Desactivar</button>}</div>
                </article>)}
              </div>
          ...
          <section className="grid gap-6 border-t border-line pt-10" aria-labelledby="reservations-title">
            <div className="flex items-end justify-between gap-4 max-phone:flex-col max-phone:items-start"><div><p className="font-mono text-xs uppercase tracking-brand text-gold">Agenda</p><h2 id="reservations-title" className="mt-2 font-display text-4xl font-medium tracking-heading text-cream">Reservas</h2></div><p className="text-sm text-muted">{reservations.length} registros</p></div>
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
              <div className="min-w-0">
                <div className="hidden border border-line bg-note xl:block">
                ...
                <div className="grid gap-3 xl:hidden">
                  {reservations.map((reservation) => <article className="grid gap-4 border border-line bg-note p-4 text-cream" key={reservation.id}>
                ...
                {reservations.length === 0 && <p className="border border-line bg-note px-5 py-10 text-center text-sm text-muted">No hay reservas para mostrar.</p>}
              </div>
